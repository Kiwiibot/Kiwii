...
final reasonCommand = ChatCommand(
  ...
  id('reason', (
    ChatContext ctx,
    @Name('case', {Locale.de: 'fall', Locale.fr: 'cas'})
    ...
    int caseId,
    @Name('reason', {Locale.de: 'grund', Locale.fr: 'raison'})
    @Description('The reason to set', {Locale.de: 'Der Grund, der festgelegt werden soll', Locale.fr: 'La raison à définir'})
    @Autocomplete(reasonAutoComplete)
    String reason, [
    @Name('last-case', {Locale.de: 'letzter-fall', Locale.fr: 'dernier-cas'})
    @Description('The last case to change', {Locale.de: 'Der letzte Fall, der geändert werden soll', Locale.fr: 'Le dernier cas à modifier'})
    @Autocomplete(caseAutoCompleteNoHistory)
    @UseConverter(IntConverter(min: 1))
    int? lastCaseId,
  ]) async {
    ...
    if (lastCaseId != null) {
      ...
      final changeButton = ButtonBuilder.primary(customId: changeId.toString(), label: ctx.guild.t.moderation.buttons.reason);
      final cancelButton = ButtonBuilder.secondary(customId: cancelId.toString(), label: ctx.guild.t.general.buttons.cancel);

      caseLow = await ctx.client.repositories.cases.getOrNull(low, ctx.guild!.id);
      caseUp = await ctx.client.repositories.cases.getOrNull(up, ctx.guild!.id);

      if (caseLow == null || caseUp == null) {
        await ctx.respond(MessageBuilder(content: ctx.guild.t.moderation.common.errors.caseRange(lower: low, upper: up), components: []));
        return;
      }
      ...
      final msg = await ctx.respond(
        MessageBuilder(
          content: ctx.guild.t.moderation.reason.pendingMultiple(
            lower: hyperlink('`#$low`', messageLink(caseLow.logMessageId!, modLogChannel, ctx.guild!.id)),
            upper: hyperlink('`#$up`', messageLink(caseUp.logMessageId!, modLogChannel, ctx.guild!.id)),
            n: up - low + 1,
          ),
          components: [
            ActionRowBuilder(components: [changeButton, cancelButton]),
          ],
        ),
      );
      ...
      if (context.parsedComponentId == cancelId) {
        await context.interaction.respond(MessageBuilder(content: ctx.guild.t.moderation.reason.canceled, components: []), updateMessage: true);
        return;
      }

      await context.acknowledge();
    } else {
      caseLow = await ctx.client.repositories.cases.getOrNull(caseId, ctx.guild!.id);

      if (caseLow == null) {
        await ctx.respond(MessageBuilder(content: ctx.guild.t.moderation.common.errors.caseNotFound(caseId: caseId), components: []));
        return;
      }
    }
    ...
    for (int i = low; i <= up; i++) {
      try {
        final case_ = await ctx.client.repositories.cases.getOrNull(i, ctx.guild!.id);
        ...
        cases.add(
          await updateCase(UpdateCase(caseId: case_.caseId, reason: Some(reason), guildId: ctx.guild!.id, actionExpiration: Some(case_.actionExpiration))),
        );
        succeeded.add(case_.caseId);
      } catch (_) {
        // Do nothing
      }
    }
    ...
    final message =
        lastCaseId != null
            ? ctx.guild.t.moderation.reason.successMultiple(
              lower: hyperlink('#$low', messageLink(caseUp!.logMessageId!, modLogChannel, ctx.guild!.id)),
              upper: hyperlink('#$up', messageLink(caseUp.logMessageId!, modLogChannel, ctx.guild!.id)),
              succeded: succeeded.length,
              n: up - low + 1,
            )
            : ctx.guild.t.moderation.reason.success(caseId: up);

    await context.respond(MessageBuilder(content: cutText(message, 1000)));
  }),
