...
Future<void> acknowledgeCase(Guild guild, Case ccase, String prefix, [User? user]) async {
  ...
  final t = guild.t;
  final modLogChannelId = (await client.repositories.guilds.get(guild.id)).modLogChannelId;
  ...
  if (ccase.logMessageId != null) {
    ...
    await message.edit(MessageUpdateBuilder(embeds: [embed]));
  } else {
    final logMessage = await channel.sendMessage(MessageBuilder(embeds: [embed]));

    await client.repositories.cases.update(ccase.toUpdate(logMessageId: Some(logMessage.id)));
  }
  ...
  if (ccase.logDmMessageId != null && ccase.action == CaseAction.ban) {
    ...
    await message.edit(
      MessageUpdateBuilder(
        content:
            guild.t.moderation.ban.dm(caseId: ccase.caseId, guildName: guild.name, reason: ccase.reason ?? guild.t.moderation.common.noReason) +
    ...
    await client.repositories.cases.update(ccase.toUpdate(logDmMessageId: Some(dmMessage.id)));
  }
}
