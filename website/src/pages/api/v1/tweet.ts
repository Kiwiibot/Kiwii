...
export const GET: APIRoute = async (context) => {
  ...
  if (user === null || text === null) {
    return new Response(
      JSON.stringify({
        errorCode: 400,
        error: "Bad request",
        message: "`user` and `text` are required params",
      }),
  ...
  ctx.font = fonts["ChirpRegular.ttf"]?.toCanvasString(23) ?? "23px sans-serif";
  const lines = canvasUtils.wrapText(ctx, text, 710, true);
  const metrics = ctx.measureText(lines.join("\n"));
  const linesLen =
    metrics.actualBoundingBoxAscent + metrics.actualBoundingBoxDescent + 15;
  ...
  ctx.font = fonts["ChirpBold.ttf"]?.toCanvasString(18) ?? "18px sans-serif";
  ctx.fillStyle = "white";
  ...
  ctx.font = fonts["ChirpRegular.ttf"]?.toCanvasString(17) ?? "17px sans-serif";
  ...
  ctx.fillStyle = "white";
  ctx.font = fonts["Noto-Bold.ttf"]?.toCanvasString(18) ?? "18px sans-serif";
  ...
  ctx.font = fonts["Noto-Regular.ttf"]?.toCanvasString(16) ?? "16px sans-serif";
...
async function fillTextWithEmoji(
  ...
  for (let currentLine = 0; currentLine < wrapped.length; currentLine++) {
    ...
    for (let i = 0; i < lineNoEmoji.length; i++) {
      ...
      const body = await fetch(parsedEmoji[0]!.url).then((res) =>
        res.arrayBuffer()
      );
